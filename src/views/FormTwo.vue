        ...
        <div class="row">           
            <div class="col">
              ...
              <b-form>
                    ...
                    <label for="especialidadeField">Especialidade principal*</label>
                      <select class="form-control" id="especialidadeField" v-model="form.especialidade" @change="$v.form.especialidade.$touch()">
                        ...
                        <option  v-for="(item, index) in especialidades" :key="index" >
                         ...
                         <span v-if="$v.form.especialidade.$error" class="error" >Campo obrigatório</span>
                  </div>
                ...
                <span v-if="$v.form.valor.$error && !$v.form.valor.required" class="error" >Campo obrigatório</span>
                <span v-if="$v.form.valor.required  && $v.form.valor.$invalid" class="error" >Digite um valor entre R$30.00 e R$350.00</span>
               ...
               >
                    <b-form-group class="my-form-group" v-if="seContem('Cartão de crédito')" label="Parcelamento em" v-slot="{ ariaDescribedby }">
                      <b-form-radio v-model="form.parcelas"  @change="$v.form.parcelas.$touch()" :aria-describedby="ariaDescribedby" name="some-radios" value="1x">1x, sem juros</b-form-radio>
                      <b-form-radio v-model="form.parcelas" @change="$v.form.parcelas.$touch()" :aria-describedby="ariaDescribedby" name="some-radios" value="2x">2x, sem juros</b-form-radio>
                      <b-form-radio v-model="form.parcelas" @change="$v.form.parcelas.$touch()" :aria-describedby="ariaDescribedby" name="some-radios" value="3x">3x, sem juros</b-form-radio>
                       <span v-if="$v.form.parcelas.$error" class="error" >Campo obrigatório</span>
                    </b-form-group>
                </b-form-checkbox-group>
                  <span v-if="$v.form.pagamento.$error" class="error" >Campo obrigatório</span>
    ...
    data() {
      return {
       ...
       formasDePagamento: [
          ...
          { item: 'Pix', name: 'Pix' },
          { item: 'Cartão de crédito', name: 'Cartão de crédito'},
        ]
    }
    },
      ...
      mandar(e){  
        e.preventDefault();
         if (!this.$v.$invalid) {
          ...
          this.$router.push({name: "FormThree"})}
        else {
          this.$v.$touch()
        }
